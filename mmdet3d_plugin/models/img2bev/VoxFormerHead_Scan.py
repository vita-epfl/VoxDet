...
# ---------------------------------------------
# Unofficial implementation of CVPR 2025:
...
class VoxFormerHead_Scan(nn.Module):
    def __init__(
        self,
        *args,
        volume_h,
        ...
        **kwargs
        ...
        self.dep_enh = ScanModule(dim=self.embed_dims)
    ...
    def forward(self, mlvl_feats, proposal, cam_params, lss_volume=None, img_metas=None,  **kwargs):
        ...
        vox_feats_diff = vox_feats_diff.permute(1, 2, 3, 0) 
        vox_feats_diff = self.dep_enh(vox_feats_diff).permute(3, 0, 1, 2).unsqueeze(0) # (B, C, H, W)

        return vox_feats_diff

class ScanModule(nn.Module):
    def __init__(self, dim=128):
        super(ScanModule, self).__init__()

        self.dep_block = ScanBlock(dim)
    def forward(self, x):
        X, Y, Z, C = x.size()

        x_dep = x.permute(1, 2, 0, 3).flatten(0, 1)  # (Y * Z, X, C)
        dep_attn_mask = torch.triu(torch.ones(X, X), diagonal=1).to(x.device)
        dep_attn_mask[:, X // 2:] = False  # depth-axis margin region
        x_dep = self.dep_block(x_dep, dep_attn_mask)
        x_dep = x_dep.reshape(Y, Z, X, C).permute(2, 0, 1, 3)  # (X, Y, Z, C)
        return x_dep
...
class ScanBlock(nn.Module):
    def __init__(self, dim):
        ...
        self.masked_sa = nn.MultiheadAttention(embed_dim=dim, num_heads=8, batch_first=True)
    ...
    def forward(self, x, attn_mask):
        ...
        attn_output, _ = self.masked_sa(x_norm1, x_norm1, x_norm1, attn_mask=attn_mask)
        x = x + attn_output

        # Feed Forward Network
        x_norm2 = self.norm2(x)
        x_ff = self.activation(self.ff1(x_norm2))
        x_ff = self.ff2(x_ff)
        x = x + x_ff
